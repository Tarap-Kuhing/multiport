...
VALIDITY () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/${GitUser}/Profile/main/Tarap-Kuhing | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/${GitUser}/Profile/main/Tarap-Kuhing | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
